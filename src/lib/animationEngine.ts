    ...
    switch (step.type) {
      case "type":
        ...
        if (timeMs < currentTimeMs + stepDurationMs) {
          ...
          return {
            text: currentText,
            cursorVisible,
            backgroundFrame: Math.floor(timeMs / 16),
          };
        }
      ...
      case "delete":
        ...
        currentText = currentText.substring(
          0,
          Math.max(0, currentText.length - step.count),
        );
        break;
      ...
      case "prepend":
        ...
        if (timeMs < currentTimeMs + stepDurationMs) {
          const progress = timeMs - currentTimeMs;
          const charsTyped = Math.floor(progress / MS_PER_CHAR_TYPE);
          const prependedText = step.text.substring(
            step.text.length - charsTyped - 1,
          );
          return {
            text: prependedText + currentText,
            cursorVisible,
            backgroundFrame: Math.floor(timeMs / 16),
          };
        }
...
export function getDefaultConfig(): AnimationConfig {
  return {
    ...
    socials: [
      ...
      { type: "ens", handle: "0xkniraj.base", enabled: true },
    ],
    font: {
      family: "mono",
      size: 24,
