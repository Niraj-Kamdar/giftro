...
describe("animationEngine", () => {
  ...
  describe("generateAnimationSteps", () => {
    ...
    it("generates type step for name with space", () => {
      ...
      expect(steps[1]).toEqual({ type: "type", text: " Niraj Kamdar" });
    });
    ...
    it("generates delete step before typing role", () => {
      ...
      expect(deleteStep.type === "delete" && deleteStep.count).toBe(13); // " Niraj Kamdar" length
    });
    ...
    it("generates steps for enabled socials", () => {
      const config: AnimationConfig = {
        ...getDefaultConfig(),
        role: "",
        socials: [{ type: "x", handle: "0xkniraj", enabled: true }],
      };
      const steps = generateAnimationSteps(config);
    ...
    it("skips socials with empty handles", () => {
      const config: AnimationConfig = {
        ...getDefaultConfig(),
        role: "",
        socials: [{ type: "x", handle: "", enabled: true }],
      };
      const steps = generateAnimationSteps(config);
    ...
    it("generates steps for all social types", () => {
      ...
      expect(
        steps.some((s) => s.type === "type" && s.text.includes("x.com/")),
      ).toBe(true);
      expect(
        steps.some((s) => s.type === "type" && s.text.includes(".sol")),
      ).toBe(true);
      expect(
        steps.some((s) => s.type === "type" && s.text.includes(".eth")),
      ).toBe(true);
      expect(
        steps.some(
          (s) => s.type === "type" && s.text.includes("youtube.com/@"),
        ),
      ).toBe(true);
      expect(
        steps.some((s) => s.type === "type" && s.text.includes("github.com/")),
      ).toBe(true);
    });
