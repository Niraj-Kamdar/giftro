...
export function InputForm({ config, onChange }: InputFormProps) {
  ...
  const updateCompression = <K extends keyof CompressionSettings>(
    key: K,
      ...
      <section>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {([0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4] as const).map((speed) => (
                <button
                  key={speed}
                  onClick={() =>
                    updateGif("playbackSpeed", speed as PlaybackSpeed)
                  }
                  className={`px-3 py-2 rounded-lg border text-sm font-medium transition-all ${
                ...
                <div className="grid grid-cols-3 gap-2">
                  {([1, 2, 3] as const).map((level) => (
                    <button
                      key={level}
                      onClick={() =>
                        updateCompression("optimizationLevel", level)
                      }
                      className={`px-3 py-2 rounded-lg border text-sm font-medium transition-all ${
